...
Запуск из корня репозитория market_bot:
  python scripts/healthcheck_data.py
  python scripts/healthcheck_data.py --strict   # exit 1 при провале обязательных проверок
...
def main() -> int:
    ...
    if failed_required:
        ...
        print("Подсказка: один раз залейте историю (spot / OI / индексы), затем запустите scripts/run_scheduler.py.")
