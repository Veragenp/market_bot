    ...
    for symbol, source in iter_config_symbols_with_source():
        ...
        try:
            result = run_human_levels_and_save(
                ...
                layer=layer,
                now_ts=now_ts,
                atr_period=HUMAN_LEVELS_ATR_PERIOD,
        ...
        logger.info("Human levels %s: zones_d1=%s zones_w1=%s", symbol, len(result.zones_d1), len(result.zones_w1))
        time.sleep(0.05)
