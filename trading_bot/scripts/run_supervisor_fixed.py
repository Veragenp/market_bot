...
def _log_supervisor_config_banner() -> None:
    ...
    logger.info(
        ...
        "BYBIT_USE_DEMO=%s BYBIT_BASE_URL=%s | LEVEL_CROSS min=%s timeout_min=%s |%s%s",
...
def _should_skip_scheduled_structural() -> tuple[bool, str]:
    ...
    try:
        ...
        if frozen and phase in ("arming", "in_position"):
            ...
            if last_transition and now - last_transition > 3600:  # 24 часа
...
def _run_data_refresh() -> Dict[str, int]:
    ...
    # TEST_MODE optimization: skip data refresh if enabled
    if st.TEST_MODE and st.TEST_MODE_SKIP_DATA_REFRESH:
        logger.info("TEST_MODE: Skipping all data refresh steps (using cached data from DB)")
        end = int(time.time())
        _stage_event(
            stage="DATA_REFRESH",
            status="skipped",
            cycle_id=run_id,
            run_id=run_id,
            message="Supervisor data refresh skipped (test mode optimization)",
            details={"skipped": True, "reason": "test_mode_skip_data_refresh"},
            started_at=start,
            finished_at=end,
        )
        return {
...
def _run_levels_rebuild() -> Dict[str, int]:
    ...
    # TEST_MODE optimization: skip levels rebuild if enabled
    if st.TEST_MODE and st.TEST_MODE_SKIP_LEVELS_REBUILD:
        logger.info("TEST_MODE: Skipping vp_local rebuild (using cached levels from DB)")
        end = int(time.time())
        _stage_event(
            stage="LEVELS_REBUILD",
            status="skipped",
            cycle_id=run_id,
            run_id=run_id,
            message="Supervisor levels rebuild skipped (test mode optimization)",
            details={"skipped": True, "reason": "test_mode_skip_levels_rebuild"},
            started_at=start,
            finished_at=end,
        )
    ...
    if (
        ...
        and not (st.TEST_MODE and st.TEST_MODE_SKIP_VP_EXPORT)
    ):
        try:
